...
boj.PHS = (function () {
  ...
  let sim_mod;
  let sim_time_current;
  let sim_time_step_start;
  let sim_time_step_end;
  ...
  function var_reset() {
    ...
    sim_mod = NaN;
    sim_time_current = NaN;
    sim_time_step_end = NaN;
  ...
  const par_spec_sim = [['accl', 'int', function () {
    return body.accl;
  }, function (val) {
    body.accl = val === 0 ? 0 : 100;
    par_sim_mod = true;
  }, {
    def_val: 100,
    min: 0,
  ...
  const par_swarm_sim = pdo.new_par_swarm('phs_sim').create_parameter(
      par_spec_sim);

  let par_step_mod = false;

  const par_spec_step = [['post', 'int', function () {
    return body.posture;
  }, function (val) {
    body.posture = val;
    par_step_mod = true;
    ...
    return move.walk_dir_in;
  }, function (val) {
    move.walk_dir_in = val;
    par_step_mod = true;
  }, {
    def_val: NaN,
    min: 0,
    ...
    return sim_time_step_end;
  }, function (val) {
    sim_time_step_end = val;
    par_step_mod = true;
  }, {
    def_val: 10,
    min: 10,
  ...
  const par_swarm_step = pdo.new_par_swarm('phs_step').create_parameter(
      par_spec_step);
  ...
  function calc_sim_const() {
    ...
    body.Adu = 0.202 * Math.pow(body.weight, 0.425) * Math.pow(body.height,
        0.725);
  ...
  function calc_step_const() {
    ...
    if (sim_mod === 1 || sim_mod === 3) { // iso7933 ver. 1
      ...
      alert('ERROR in calc_step_const ' + sim_mod);
    }
    if (body.accl >= 50) {
      sweat.SWmax = sweat.SWmax * 1.25;
    }
    ...
    if (!isNaN(move.v_walk_in)) {
      ...
      if (!isNaN(move.walk_dir_in)) {
        move.v_air_rel = Math.abs(air.v_air - move.v_walk * Math.cos(
            3.14159 * move.walk_dir_in / 180));
      } else {
    ...
    heatex.Tresp = 28.56 + 0.115 * air.Tair + 0.641 * air.Pw_air;
    heatex.Cresp = 0.001516 * body.Met * (heatex.Tresp - air.Tair);
    heatex.Eresp = 0.00127 * body.Met * (59.34 + 0.53 * air.Tair - 11.63
        * air.Pw_air);
    if (move.v_air_rel > 1) {
      heatex.Z = 8.7 * Math.pow(move.v_air_rel, 0.6);
    } else {
      heatex.Z = 3.5 + 5.2 * move.v_air_rel;
    }
    par_step_mod = false;
  }
  ...
  function step_core_temp_equi_mr() {
    const Tcreqm = core.Tcreq_rm_ss;
    const ConstTeq = core.Tcreq_mr_ConstTeq;
    const val_0 = core.Tcreq_mr_0;
    core.Tcreq_mr = val_0 * ConstTeq + Tcreqm * (1 - ConstTeq);
    core.dStoreq = body.spHeat * (core.Tcreq_mr - val_0) * (1
        - core.sk_cr_rel_0);
  }

  function skin_temp_equilibrium() {
    const Tskeq_cl = 12.165 + 0.02017 * air.Tair + 0.04361 * air.Trad + 0.19354
        * air.Pw_air - 0.25315 * air.v_air + 0.005346 * body.Met + 0.51274
        * core.Trec;
    const Tskeq_nu = 7.191 + 0.064 * air.Tair + 0.061 * air.Trad + 0.198
        * air.Pw_air - 0.348 * air.v_air + 0.616 * core.Trec;
    const I_cl = cloth.Icl;
    if (I_cl <= 0.2) {
      return Tskeq_nu;
    }
    if (I_cl >= 0.6) {
  ...
  function calc_dynamic_insulation() {
    let Vaux = move.v_air_rel;
    if (move.v_air_rel > 3) {
      Vaux = 3;
    }
    let Waux = move.v_walk;
    if (move.v_walk > 1.5) {
      Waux = 1.5;
    }
    cloth.CORcl = 1.044 * Math.exp(
        (0.066 * Vaux - 0.398) * Vaux + (0.094 * Waux - 0.378) * Waux);
    if (cloth.CORcl > 1) {
      cloth.CORcl = 1;
    }
    cloth.CORia = Math.exp(
        (0.047 * move.v_air_rel - 0.472) * move.v_air_rel + (0.117 * Waux
            - 0.342) * Waux);
    if (cloth.CORia > 1) {
      cloth.CORia = 1;
    }
    ...
    if (cloth.Icl <= 0.6) {
      cloth.CORtot = ((0.6 - cloth.Icl) * cloth.CORia + cloth.Icl * cloth.CORcl)
          / 0.6;
    }
    cloth.Itot_dyn = cloth.Itot_st * cloth.CORtot;
    const IAdyn = cloth.CORia * cloth.Ia_st;
    cloth.Icl_dyn = cloth.Itot_dyn - IAdyn / cloth.fAcl;

    cloth.CORe = (2.6 * cloth.CORtot - 6.5) * cloth.CORtot + 4.9;
    cloth.im_dyn = cloth.im_st * cloth.CORe;
    if (cloth.im_dyn > 0.9) {
      cloth.im_dyn = 0.9;
    }
    cloth.Rtdyn = cloth.Itot_dyn / cloth.im_dyn / 16.7;
  }

  function dynamic_convection_coefficient() {
    heatex.Hcdyn = 2.38 * Math.pow(Math.abs(skin.Tsk - air.Tair), 0.25);
    if (heatex.Z > heatex.Hcdyn) {
      heatex.Hcdyn = heatex.Z;
    }
    cloth.fAcl_rad = (1 - cloth.fAref) * 0.97 + cloth.fAref * cloth.Fr;
  }

  function clothing_temp() {
    const Hcdyn = heatex.Hcdyn;
    let Trad_K_pow4 = air.Trad + 273;
    ...
    while (loop > 0) {
      loop--;
      let Tcl_K_pow4 = cloth.Tcl + 273;
      Tcl_K_pow4 = Tcl_K_pow4 * Tcl_K_pow4;
      Tcl_K_pow4 = Tcl_K_pow4 * Tcl_K_pow4;
      heatex.Hr = cloth.fAcl_rad * body.fAdu_rad_aux * (Tcl_K_pow4
          - Trad_K_pow4) / (cloth.Tcl - air.Trad);
      Tcl1 = ((cloth.fAcl * (Hcdyn * air.Tair + heatex.Hr * air.Trad) + skin.Tsk
          / cloth.Icl_dyn)) / (cloth.fAcl * (Hcdyn + heatex.Hr) + 1
          / cloth.Icl_dyn);
  ...
  function sweat_rate() {
    ...
    sweat.SWpre = sweat.SWpre * sweat.ConstSW + sweat.SWreq * (1
        - sweat.ConstSW);
    if (sweat.SWpre <= 0) {
      sweat.Epre = 0;
      sweat.SWpre = 0;
      ...
      const k = sweat.Emax / sweat.SWpre;
      skin.w_pre = 1;
      if (k >= 0.5) {
    ...
    //trace if ( sim_time_current <=5 || (sim_time_current % 40) === 0 ){
    //trace     console.log( JSON.stringify(
    //trace         {time: sim_time_current, Ereq: sweat.Ereq, Eeff_req: sweat.Eeff_req,
  ...
  function core_temp_pred_std() {
    const dStorage = sweat.Ereq - sweat.Epre + core.dStoreq;
    ...
    while (loop > 0) {
      ...
      if (core.sk_cr_rel > 0.3) {
        core.sk_cr_rel = 0.3;
      }
      if (core.sk_cr_rel < 0.1) {
        core.sk_cr_rel = 0.1;
      }
      core.Tcr = dStorage / body.spHeat + skin.Tsk_0 * core.sk_cr_rel_0 / 2
          - skin.Tsk * core.sk_cr_rel / 2;
      core.Tcr = (core.Tcr + core.Tcr_0 * (1 - core.sk_cr_rel_0 / 2)) / (1
          - core.sk_cr_rel / 2);
  ...
  function core_temp_pred_Tcr(Tbm, Tcr_0, Tsk) {
    ...
    console.error('core_temp_pred overun' + sim_time_current + Tcr);
    return Tcr;
  }

  function core_temp_pred() {
    const dStorage = sweat.Ereq - sweat.Epre + core.dStoreq;
    const dTempStorage = dStorage / body.spHeat;
    body.Tbm = body.Tbm_0 + dTempStorage;
    const rv = core_temp_pred_Tcr(body.Tbm, core.Tcr_0, skin.Tsk);
    core.Tcr = rv;
  }

  function rect_temp_pred() {
    const Tre0 = core.Trec_0;
    core.Trec = Tre0 + (2 * core.Tcr - 1.962 * Tre0 - 1.31) / 9;
    if (!limit.rec_temp && core.Trec >= 38) {
      limit.rec_temp = sim_time_current;
    }
  }
  ...
  function water_loss() {
    ...
    const k_to_g = 2.67 * body.Adu / 1.8 / 60;
    sweat.SWg = sweat.SW * k_to_g;
    sweat.SWtotg = sweat.SWtot * k_to_g;
    if (!limit.water_loss_max50 && sweat.SWtotg >= limit.sweat_max50) {
      limit.water_loss_max50 = sim_time_current;
    }
    if (!limit.water_loss_max95 && sweat.SWtotg >= limit.sweat_max95) {
      limit.water_loss_max95 = sim_time_current;
    }
  }

  function set_start_time() {
    sim_time_step_start = sim_time_current;
  }

  function state() {
    return {
      current_time: sim_time_current,
      step_start_time: sim_time_step_start,
      step_end_time: sim_time_step_end,
      Tcreq: core.Tcreq_mr,
      Tsk: skin.Tsk,
      SWg: sweat.SWg,
      SWtotg: sweat.SWtotg,
  ...
  function sample_get() {
    ...
    const res = {
      sim_time: sim_time_current, sim_mod: sim_mod
    };
    let idp;
    let tag;
    let val;
  ...
  return {
    par_spec_sim: par_spec_sim,
    par_spec_step: par_spec_step,
    par_swarm_sim: par_swarm_sim,
    par_swarm_step: par_swarm_step,
    reset: reset,
    sim_init: sim_init,
    state: state,
    time_step: time_step,
