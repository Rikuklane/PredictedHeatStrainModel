...
boj.PHS = ( function () {
    ...
    var par_swarm_step = pdo.new_par_swarm( 'phs_step' ).
        create_parameter( par_spec_step );

    var branch_reg = {};
    var flag = 0;

    function branch( id ){
        if ( branch_reg[ id ]){ branch_reg[ id ] ++; }
        else { branch_reg[ id ] = 1; }
    }

    function branch_reset(){
        var res = JSON.stringify( branch_reg );
        branch_reg = {};
        return res;
    }
