/**
 * PHS Run Simulation Module
 * Handles simulation execution, data logging, and result management
 * Refactored to use modern API with improved readability
 */
"use strict;";
var boj = boj || {};
...
boj.PHS_run_sim = (function () {
  ...
  // Modern API references
  const phs = boj.PHS;
  const pdo = boj.par_data_object;
  const dtab = boj.data_table_obj;
  const diu = boj.diagram_util;

  // Use modern parameter specifications
  const parameterSpecsSimulation = phs.parameterSpecs.simulation;
  const parameterSpecsStep = phs.parameterSpecs.step;
  const parameterSwarmsSimulation = phs.parameterSwarms.simulation;
  const parameterSwarmsStep = phs.parameterSwarms.step;

  // Create parameter reference for unified access
  const parref = parameterSwarmsSimulation.new_parref(parameterSwarmsStep);
  const par_get = parref.get_value_func();
  const par_set = parref.set_value_func();

  /**
 * Creates a new run simulation instance
 * @returns {run_sim} New simulation runner object
 */
...
/**
 * Run Simulation Constructor
 * Manages PHS simulation execution, data collection, and result formatting
 */
function run_sim() {
  // Initialize simulation state
  this.run_log = [];
  this.sim_inp_tab = null;
  this.step_inp_tab = null;
  this.step_log_tab = null;
  this.sim_res_tab = null;
  this.sim_target_tab = null;
}
...
/**
 * Simulation input parameters (subject characteristics)
 * Defines which simulation-level parameters to track and display
 */
const sim_inp_pid = []; // [ pid, ... ]

const sim_inp_tab_spec = [
  ['accl', null, 0], 
  ['drink', null, 0],
  ['height', null, 2], 
  ['mass', null, 2], 
...
/**
 * Step input parameters (environmental conditions per time step)
 * Defines which step-level parameters to track and display
 */
const step_inp_pid = []; // [ pid, ... ]

const step_inp_tab_spec = [
  ['step_end_time', 'step_end_time', 0], 
  ['post', 'post', 0],
  ['Tair', 'Tair', 1], 
  ['Pw', 'Pw_air', 1], 
  ['Trad', 'Trad', 1],
...
/**
 * Step log parameters (physiological results per time step)
 * Defines which physiological variables to log during simulation
 */
const step_log_pid = []; // [ pid, ... ]

const step_log_tab_spec = [
  ['time', null, 0], 
  ['Tcreq', null, 2],
  ['Tsk', null, 2], 
  ['SWg', null, 2], 
 ...
 * Defines which final simulation results to track and display
 */
const sim_res_pid = [];  // [ pid, ... ]

const sim_res_tab_spec = [
  ['time', 'time', 0], 
  ['Tre', 'Tre', 3],
  ['D_Tre', 'D_Tre', 0], 
  ['SWtotg', 'SWtotg', 0], 
...
/**
 * Simulation target results (reference values for comparison)
 * Defines target values for simulation validation and comparison
 */
const sim_target_pid = [];  // [ pid, ... ]
const sim_target_pid_default = [];

const sim_target_tab_spec = [
  ['time', 'time', 0], 
  ['Tre', 'Tre', 3],
  ['D_Tre', 'D_Tre', 0], 
  ['SWtotg', 'SWtotg', 0], 
...
/**
 * Simulation Input Table Functions
 * Handles creation and management of simulation parameter input tables
 */

/**
 * Adds columns to simulation input data table
 * @param {Object} tab - Data table object to add columns to
 */
...
/**
 * Creates a new simulation input data table object
 * @returns {Object} New data table configured for simulation inputs
 */
...
/**
 * Fills simulation input table with data from the latest simulation run
 * Uses the run log to populate the table with simulation parameters
 */
function sim_inp_tab_fill() {
  ...
  /**
 * Converts simulation input table to HTML format
 * @param {HTMLElement} node - DOM element to append table to
...
/**
 * Converts simulation input table to CSV format
 * @returns {string} CSV representation of the simulation input data
 */
...
// ============================================================================
// STEP INPUT TABLE FUNCTIONS
// ============================================================================

/**
 * Step Input Table Functions
 * Handles creation and management of time step parameter input tables
 */

/**
 * Adds columns to step input data table
 * @param {Object} tab - Data table object to add columns to
 * @param {boolean} notime - Whether to exclude the time column
 */
...
/**
 * Creates a new step input data table object
 * @returns {Object} New data table configured for step inputs
 */
function step_inp_dtab_obj() {
  ...
  /**
 * Fills step input table with time series data from simulation runs
 * Creates rows for each time step with environmental parameters
 */
function step_inp_tab_fill() {
  ...
  // Process each simulation run to create time series data
  for (let ix = 0; ix < logs.length; ix++) {
    const run = logs[ix];
    const row = run[2].slice(0); // make a copy of step parameters
    
    // Add step end time from simulation results
    // run[3] contains sim_res where first element (index 0) is the end time
    if (run[3] && run[3][0] !== undefined) {
      row.unshift(run[3][0]); // use step end time from simulation results
    } else if (tab.data !== undefined && tab.data.length > ix) {
      row.unshift(tab.data[ix][0]); // fallback: get previous time if any
    } else {
      row.unshift(phs.state().step_end_time); // fallback: use current step end time from PHS state
    }
  ...
  /**
 * Converts step input table to HTML format
 * @param {HTMLElement} node - DOM element to append table to
 * @param {boolean} append - Whether to append or replace content
 */
...
/**
 * Converts step input table to CSV format
 * @returns {string} CSV representation of the step input data
 */
run_sim.prototype.step_inp_tab_to_csv = function () {
  ...
  run_sim.prototype.step_log_to_graph = function (html_dst) {
    // Check if the graph element exists (required for tests)
    if (!html_dst) {
      console.warn('Graph element not found - skipping graph rendering');
      return;
    }
