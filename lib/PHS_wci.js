...
boj.PHS_wci = (function (document) {
  ...
  function updateButtonStates(from_start) {
    // Get the actual buttons by ID
    const startButton = document.getElementById('startSimBtn');
    const addButton = document.getElementById('addTimestepBtn');
    const importButton = document.getElementById('importBtn');
    
    // Get export dropdown button
    const exportDropdownBtn = document.querySelector('.export-dropdown-btn');
    
    if (!startButton || !addButton || !importButton) {
      console.log('Buttons not found for state management');
      return;
    }
    
    if (from_start) {
      // First simulation - disable start and import buttons, enable add button
  ...
  function exportAllData() {
    try {
      // Extract step log data directly (output data)
      let stepLogData = [];
      ...
      XLSX.utils.book_append_sheet(wb, ws2, "Output");
      
      // Save Excel file
      XLSX.writeFile(wb, 'PHS_output_data.xlsx');
      console.log('Output data exported to Excel');
    } catch (error) {
      console.error('Error exporting output data:', error);
      alert('Error exporting output data: ' + error.message);
    }
  }
  ...
  function exportAllDataCombined() {
    try {
      ...
      // Check if we have any data
      if (allInputData.length === 0 && stepLogData.length === 0) {
        throw new Error('No simulation data found. Please run a simulation first.');
      }
    ...
    input.onchange = function(e) {
      ...
      reader.onload = function(e) {
        try {
          ...
          // Look for "Input" sheet
          if (!workbook.Sheets['Input']) {
            throw new Error('No "Input" sheet found in the Excel file. Please use a file exported from this application.');
          }
          ...
          if (jsonData.length < 2) {
            throw new Error('Input sheet is empty or has no data rows.');
          }
          
          // Parse the imported data
          processImportedData(jsonData);
          
          alert('Data imported successfully! You can now run the simulation.');
        } catch (error) {
  ...
  function processImportedData(data) {
    ...
    if (simParamEndIndex === -1) {
      throw new Error('Could not find step_end_time column. Invalid input file format.');
    }
    
    // Simulation parameters are before step_end_time
    const simHeaders = headers.slice(0, simParamEndIndex);
    const stepHeaders = headers.slice(simParamEndIndex);
    
    // Get first data row for simulation parameters
    if (data.length < 2) {
      throw new Error('No data rows found in input file.');
    }
    
    const firstRow = data[1];
    
    // Set simulation parameters (these are the same for all steps)
    simHeaders.forEach((header, index) => {
      const value = firstRow[index];
      if (value !== undefined && value !== null && value !== '') {
        try {
          phsinp.par_wci_obj.par_set(header, parseFloat(value));
        } catch (error) {
    ...
    for (let rowIndex = 1; rowIndex < data.length; rowIndex++) {
      ...
      stepHeaders.forEach((header, colOffset) => {
        ...
        if (value !== undefined && value !== null && value !== '') {
          try {
            // Map column headers to actual parameter names
            const paramMapping = {
            ...
            phsinp.par_wci_obj.par_set(paramName, parseFloat(value));
          } catch (error) {
            console.warn(`Could not set step parameter ${header} for row ${rowIndex}:`, error);
          }
        }
      });
      ...
      run_sim.run_simulation(`imported_step_${rowIndex}`, isFirstStep);
    }
  ...
  const self = {
    ...
    exportAllDataCombined: exportAllDataCombined,
    importInputData: importInputData,
