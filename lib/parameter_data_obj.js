...
/**
 * Enhanced Parameter Data Object Module
 * Provides parameter management with improved organization and documentation
 */
boj.par_data_object = (function () {

  /**
   * Utility function for prototype inheritance
  ...
  /**
   * Creates inheritance chain between prototypes
   * @param {Object} proto - Prototype to inherit from
   * @returns {Object} New object with inherited prototype
   */
  ...
  /**
   * Base parameter class - all parameter types inherit from this
   * @param {string} parameterId - Unique identifier for the parameter
   */
  function Par_any(parameterId) {
    this.pid = parameterId;
  }

  /**
   * Parameter type constructors mapping
   */
  const parameterConstructors = {
    'int': Par_int, 
    'float': Par_float, 
    'string': Par_string, 
    'bool': Par_bool
  ...
  /**
   * Factory function to create parameters by type
   * @param {string} type - Parameter type ('int', 'float', 'string', 'bool')
   * @param {string} parameterId - Parameter identifier
   * @returns {Object} New parameter instance
   */
  function createParameter(type, parameterId) {
    const constructor = parameterConstructors[type];
    return constructor && new constructor(parameterId);
  }

  /**
   * Valid configuration options for parameters
   */
  const validConfigOptions = ['dec_nof', 'def_val', 'descr', 'min', 'max',
    'output', 'symbol', 'unit'];

  /**
   * Configure parameter with provided options
   * @param {Object} configObject - Configuration object
   */
  Par_any.prototype.configure = function (configObject) {
    const self = this;
    if (configObject) {
      validConfigOptions.map(function (optionId) {
        if (optionId in configObject) {
          self[optionId] = configObject[optionId];
        }
      });
    }
    return self;
  ...
  Par_any.prototype.config = Par_any.prototype.configure;
  ...
  Par_any.prototype.get_info = function () {
    ...
    const self = this;
    validConfigOptions.map(function (id) {
  ...
  function get_value(val) {
    // TODO: Clean up this validation and define proper min/max values for all parameters
    // Current approach uses selective validation to avoid breaking existing functionality
    // Future work should audit all parameter limits and implement comprehensive validation
    
    // Parameters with clear logical bounds for both min and max
    const enforce_full_validation = 
      this.pid === 'sim_mod' ||           // Simulation model (0-4)
      this.pid === 'accl' ||              // Acclimatization (0-100%)
      this.pid === 'drink' ||             // Drinking (0-1)
      this.pid === 'post' ||              // Posture (1-3)
      (this.unit === '%' && this.min >= 0); // Any percentage parameter with non-negative min

    // Parameters that just shouldn't be negative (only enforce val >= 0)
    const enforce_non_negative = 
      this.pid === 'mass' ||              // Body mass (can't be negative)
      this.pid === 'height' ||            // Height (can't be negative)
      this.pid === 'Met' ||               // Metabolism (can't be negative)
      this.pid === 'Icl' ||               // Clothing insulation (can't be negative)
      this.pid === 'im_st' ||             // Static permeability (can't be negative)
      this.pid === 'fAref' ||             // Area factor (can't be negative)
      this.pid === 'Fr' ||                // Reflectivity (can't be negative)
      this.pid === 'v_air' ||             // Air velocity (can't be negative)
      this.pid === 'v_walk';              // Walking speed (can't be negative)

    // Enforce minimum values for parameters with defined bounds
    if (enforce_full_validation && 'min' in this) {
      const min = this.min;
      if (val < min) {
    ...
    // Enforce non-negative values for physical parameters
    if (enforce_non_negative && val < 0) {
      val = 0;
    }
    // Only enforce maximum values for parameters with clear upper bounds
    if (enforce_full_validation && 'max' in this) {
      const max = this.max;
      if (val > max) {
        // Enforce maximum value for parameters with clear bounds
        val = max;
      }
    }
    if (par_debug_do) {
  ...
  Par_swarm.prototype.create_parameter = function (par_spec) {
    ...
    par_spec.map(function (s) {
      const type = s[1];
      const pid = s[0];
      const par = createParameter(type, pid);
      if (!par) {
        alert(
  ...
  return {
    ...
    parameterConstructors: parameterConstructors,
    validConfigOptions: validConfigOptions,
    
    // Legacy compatibility
    new_parameter: createParameter,
    Par_constr_type: parameterConstructors,
    par_config_option_id: validConfigOptions,
